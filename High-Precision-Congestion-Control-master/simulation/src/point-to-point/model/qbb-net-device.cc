...
namespace ns3 {
    ...
    int RdmaEgressQueue::GetNextQindex(bool paused[]){
        ...
        int res = -1024;
        uint32_t fcount = m_qpGrp->GetN();//qp的数量
        ...
        QbbNetDevice::DequeueAndTransmit(void)
    {
        ...
        if (m_node->GetNodeType() == 0){ //主机端
            ...
            if (qIndex != -1024){ //主机端判断该流是否结束
                ...
                Ptr<RdmaQueuePair> lastQp = m_rdmaEQ->GetQp(qIndex); //根据queuePairGroup中保存的Ptr<RdmaQueuePair>的vector，通过qIndex的索引值，获取vector中的Ptr<RdmaQueuePair>
                p = m_rdmaEQ->DequeueQindex(qIndex);
            ...
            if (p != 0){
                ...
                uint16_t protocol = 0;
                ProcessHeader(packet, protocol);// 为什么要copy一份packet
                packet->RemoveHeader(h);
                FlowIdTag t;
                uint32_t qIndex = m_queue->GetLastQueue();
                if (qIndex == 0){//this is a pause or cnp, send it immediately!
                    m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);//将INT的信息写入到buffer中
                    p->RemovePacketTag(t);
                }else{
