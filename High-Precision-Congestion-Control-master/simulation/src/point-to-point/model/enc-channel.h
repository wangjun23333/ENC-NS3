...
#ifndef ENC_CHANNEL_H
#define ENC_CHANNEL_H
 ...
 */
class EncChannel : public PointToPointChannel 
{
public:
  static TypeId GetTypeId (void);

  /**
   * \brief Create a EncChannel
   *
   ...
   */
  void Attach (Ptr<EncNetDevice> device);

  /**
   * \brief Transmit a packet over this channel
   * \param p Packet to transmit
   * \param src Source EncNetDevice
   * \param txTime Transmit time to apply
   * \returns true if successful (currently always true)
   */
  virtual bool TransmitStart (Ptr<Packet> p, Ptr<EncNetDevice> src, Time txTime);
  ...
  /*
   * \brief Get EncNetDevice corresponding to index i on this channel
   * \param i Index number of the device requested
   * \returns Ptr to EncNetDevice requested
   */
  Ptr<EncNetDevice> GetEncDevice (uint32_t i) const;
...
protected:
   ...
   * \returns Ptr to EncNetDevice source for the 
   * specified link
   */
  Ptr<EncNetDevice> GetSource (uint32_t i) const;

  /*
   * \brief Get the net-device destination
   * \param i the link requested
   * \returns Ptr to EncNetDevice destination for 
   * the specified link
   */
  Ptr<EncNetDevice> GetDestination (uint32_t i) const;
...
public:
    ...
    WireState                  m_state;
    Ptr<EncNetDevice> m_src;
    Ptr<EncNetDevice> m_dst;
  };
